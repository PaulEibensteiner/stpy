...
class RegularizedDictionary(Estimator):
    ...
    def dist(self, tstar, delta, accuracy=1e-3, xtest=None):
        ...
        if xtest is None:
            objective = cp.sum_squares((theta - tstar.view(-1, 1)))
        else:
            objective = cp.max(cp.abs(xtest @ (theta - tstar.view(-1, 1))))
        if self.regularizer is not None:
    ...
    def mean_std(self, xtest: torch.Tensor):
        ...
        """
        warnings.warn(
            "This is only a legacy function, it does not provide uncertainty for the class" + str(__class__.__name__))
